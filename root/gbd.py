...
from lib.display.DISPLAY import getspi
...
def get_buf():
  global _ili9341fb
  return _ili9341fb._buffer

def gbd(rotation=None):
  global _spi
  global _display
  global _ili9341fb
  global _speed   
  ...
  if _display is None:
    ...
    _ili9341fb = ILI9341FB(_spi, cs=Pin(26), dc=Pin(5), rst=Pin(33), width=320, height=240, rotation=_rotation)
    _display = DisplayExt( _ili9341fb, _ili9341fb.width, _ili9341fb.height, framebuf.RGB565)
    _display.show = _ili9341fb.show
  else:
    if _rotation != _oldrotation:
      print('get new display, rotation = ', _rotation)
      _ili9341fb = ILI9341FB(_spi, cs=Pin(26), dc=Pin(5), rst=Pin(33), width=240, height=320, rotation=_rotation)
      _display = DisplayExt( _ili9341fb, _ili9341fb.width, _ili9341fb.height, framebuf.RGB565)
      _display.show = _ili9341fb.show
    else:
      _display.base.spi = _spi
...
########################################################################################################
  ...
  def show(self):
    ...
    #   return
    print( "Show screen, first 4 bytes {:02x}, {:02x}, {:02x}, {:02x}".format( self._buffer[0], self._buffer[1], self._buffer[2], self._buffer[3]))
